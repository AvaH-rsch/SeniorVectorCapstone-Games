...
while not game_over:
  ...
  if turn == 1:
    selected_column = int(input("Player 1, choose a column (0-6): ")) # takes in the answer from the player and converts it to a number 
    ...
    for row in range(board.shape[0]):          # go through each row
      for col in range(matrix.shape[1] - 3):  # check groups of 4
    ...
    for col in range(board.shape[1]):          # go through each column
      for row in range(matrix.shape[0] - 3):  # check groups of 4 vertically
        if (board[row][col] == 1 and
...
# ask for player 2 input 
  if turn == 2:
    selected_column = int(input("Player 2, choose a column (0-6): "))
    for row in range(board.shape[0]):   # go through each row
      if board[row][selected_column] == 0:     # check if the value is 0
          board[row][selected_column] = 1      # replace it with 1
          break                        # stop after replacing the first 0
